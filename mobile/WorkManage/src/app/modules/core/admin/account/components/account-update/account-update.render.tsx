...
import { IAccountUpdateProps } from "./account-update.component";
import { RoleVM, GroupVM } from "../../../../../../view-models";
import { useStyles } from "./account-update.styles";

export interface IRenderAccountUpdateProps extends IAccountUpdateProps {
...
export const RenderAccountUpdateComponent = (props: IRenderAccountUpdateProps) => (
    <Form style={{ flex: 1 }}>
            ...
            <Tab heading={<TabHeading style={{ backgroundColor: "#BEE" }}><Icon style={{ color: "#00a8cc" }} name="user-circle" type="FontAwesome5" /><Text style={{ color: "#00a8cc" }}>Tải khoản</Text></TabHeading>}>
                <Form style={useStyles().form}>
                    <Item regular
                        ...
                        <Input disabled placeholder="Tên tài khoản" value={props.useGetFormControl("UserName").value} onChangeText={value => { props.useHandleOnChange({ name: 'UserName', value }) }} />
                    </Item>
                </Form>
            </Tab>
        </Tabs>
